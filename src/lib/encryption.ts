 ...
 *
 * Encrypts OAuth access/refresh tokens before storing in database
...
export function isEncryptedToken(value: string): boolean {
  ...
  try {
    ...
    // Check if it's long enough to contain salt + iv + tag + some encrypted data
    return buffer.length >= SALT_LENGTH + IV_LENGTH + TAG_LENGTH + 16
  } catch {
    return false
  }
}
