...
const createProjectSchema = z.object({
  name: z.string().min(3).max(50).regex(/^[a-zA-Z0-9-_ ]+$/), // Alphanumeric + basic chars
...
const projects = await prisma.$queryRaw`SELECT * FROM projects WHERE userId = ${userId}`; // Still safe (template literals)
...
// Middleware
export async function rateLimit(req: NextRequest, type: keyof typeof ratelimits = 'api') {
...
module.exports = {
  async headers() {
    return [{
      source: '/api/:path*',
      headers: [
        { key: 'Access-Control-Allow-Origin', value: 'https://shipsensei.dev' }, // Specific origin, not '*'
...
async function scanGeneratedCode(code: string): Promise<SecurityIssue[]> {
  ...
  if (code.includes('eval(')) {
    issues.push({ severity: 'critical', message: 'eval() detected', line: findLine(code, 'eval(') });
  }

  if (code.match(/innerHTML\s*=\s*[^`]/)) {
    issues.push({ severity: 'high', message: 'Potential XSS via innerHTML', line: findLine(code, 'innerHTML') });
  }

  if (code.match(/\$\{[^}]+\}/)) {
    issues.push({ severity: 'medium', message: 'Template literal - ensure safe usage', line: findLine(code, '${') });
  }
